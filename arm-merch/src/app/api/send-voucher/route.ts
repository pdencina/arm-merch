      ...
      <table width="520" cellpadding="0" cellspacing="0" style="background:#111;border-radius:16px;overflow:hidden;border:1px solid #222">
        ...
        <tr><td style="padding:28px 32px 0">
          <div style="display:flex;justify-content:space-between;margin-bottom:20px">
            <div>
              <div style="font-size:11px;color:#666;text-transform:uppercase;letter-spacing:.1em;margin-bottom:4px">Orden</div>
              <div style="font-size:22px;font-weight:700;color:#fff">#${data.orderNumber}</div>
            </div>
            <div style="text-align:right">
              <div style="font-size:11px;color:#666;text-transform:uppercase;letter-spacing:.1em;margin-bottom:4px">Fecha</div>
              <div style="font-size:14px;color:#ccc">${data.date}</div>
            </div>
          </div>
        ...
        <tr><td style="padding:20px 32px">
          <div style="background:#1a1a1a;border-radius:10px;padding:16px;border:1px solid #2a2a2a">
            ...
            <div style="display:flex;justify-content:space-between;margin-bottom:8px">
              ...
              <span style="color:#ccc;font-size:13px">${fmt(data.subtotal)}</span>
            </div>
            <div style="display:flex;justify-content:space-between;margin-bottom:8px">
              <span style="color:#4ade80;font-size:13px">Descuento</span>
              <span style="color:#4ade80;font-size:13px">−${fmt(data.discount)}</span>
            </div>
            ` : ''}
            <div style="display:flex;justify-content:space-between;padding-top:${data.discount > 0 ? '10px;border-top:1px solid #2a2a2a' : '0'}">
              <span style="color:#fff;font-size:16px;font-weight:700">Total</span>
              <span style="color:#f59e0b;font-size:20px;font-weight:700">${fmt(data.total)}</span>
            </div>
            <div style="margin-top:10px;padding-top:10px;border-top:1px solid #2a2a2a;display:flex;justify-content:space-between">
              <span style="color:#555;font-size:12px">Método de pago</span>
              <span style="color:#ccc;font-size:12px;text-transform:capitalize">${data.paymentMethod}</span>
            </div>
          </div>
        </td></tr>
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data, error } = await resend.emails.send({
      from:    'ARM Merch <voucher@armglobal.cl>',
      to:      [body.to],
